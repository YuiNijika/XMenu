...
export function VehiclePage({ report }: PageProps) {
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {!value?.valid ? (
        <p className="text-xs text-muted-foreground">{t('vehicle.notInVehicle')}</p>
      ) : null}

      <Tabs defaultValue="toggles" className="w-full">
        <TabsList className="mb-1">
          <TabsTrigger value="toggles">{t('common.toggles')}</TabsTrigger>
          <TabsTrigger value="spawn">{t('vehicle.spawnVehicle')}</TabsTrigger>
          <TabsTrigger value="paint">{t('vehicle.paint')}</TabsTrigger>
        </TabsList>
            ...
            <CardContent className="flex flex-col gap-4">
              {schema ? (
                <>
                  ...
                  <SchemaSection payload={schema} report={report} tabId="vehicle" pageId="vehicleMain" sectionId="status" visible={value?.valid === true} />
                  <SchemaSection payload={schema} report={report} tabId="vehicle" pageId="vehicleMain" sectionId="proof" visible={value?.valid === true} />
                  <SchemaSection payload={schema} report={report} tabId="vehicle" pageId="vehicleMain" sectionId="health" visible={value?.valid === true} />
                  <SchemaSection payload={schema} report={report} tabId="vehicle" pageId="vehicleMain" sectionId="special" visible={value?.valid === true} />
              ...
              <Separator />
              <div className="text-sm font-medium">{t('vehicle.sectionRuntime')}</div>
              <ToggleGrid
                report={report}
                items={[
              ...
              <Separator />
              <div className="text-sm font-medium">{t('vehicle.lockSpeed')}</div>
              <div className="flex flex-wrap items-end gap-3">
                <div className="w-28">
          ...
          <div className="grid gap-5 md:grid-cols-2">
            {schema ? (
              <Card className="md:col-span-2">
                <CardHeader>
                  <CardTitle>{t('vehicle.sectionSpawnOptions')}</CardTitle>
                  <CardDescription>{t('react.uiHint')}</CardDescription>
                </CardHeader>
